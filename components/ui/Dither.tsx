...
class RetroEffectImpl extends Effect {
    ...
    update(_renderer: any, _inputBuffer: any, deltaTime: any) {
        // No per-frame update needed here for now, uniforms handled via React
    }
}
...
export default function Dither({
    waveColor = [0.1, 0.2, 0.05], // Site-inspired green base
    colorNum = 4,
    pixelSize = 2,
    waveSpeed = 0.05,
    waveFrequency = 3,
...
function DitheredScene({
    ...
    }), [size.width, size.height, waveColor, waveSpeed, waveFrequency, waveAmplitude, enableMouseInteraction, mouseRadius]);

    useFrame((state) => {
        uniforms.time.value = state.clock.getElapsedTime();
        if (enableMouseInteraction) {
            uniforms.mousePos.value.set(state.mouse.x * size.width, state.mouse.y * size.height);
        }
    });
